...
globals [
  ...
  ;IBI-list                   ; Keeps track of interbirth interval (IBI) of animals within year

  scen-outs-list             ; Scenario output list updated monthly
]
...
to setup-scenarios
  ...
  let path "EcoEvoInputs/"


  let scen 0
  if scenario = 1 [ set scen "_pos_pulses.csv" ]
  if scenario = 2 [ set scen "_neg_pulses.csv" ]
  if scenario = 3 [ set scen "_pos_and_neg_pulses.csv" ]

  if scenario != 0
  [
    let file (word "EcoEvoInputScenarios_vers_" version scen)
...
to go
  ...
  let yr-end 1
  if debug = "Eval" [ set yr-end 2 ]

  if count turtles <= 2 or year = out-year + yr-end [           ; Model stops if all animals die or end of simulation period is reached
...
to daily-tasks
  ...
  ; Scenario updates
  if debug = "Scen" and year > 9 [ update-scenarios ]
...
to give-birth
 ...
 ;     let IBI age - age-birth
     ; set IBI-list lput IBI IBI-list
 ;   ]
 ;   set age-birth age
 ; ]
  ...
  [
    ...
    repeat n-off [
      ...
      if prob-off >= surv-prob-off [ ;or mass-off <= 0 or lean-mass-off <= 0 or SL-off <= 0 [
        ...
        if debug = 10 and year = out-year [
          set age-at-death-list lput ds-birth age-at-death-list
...
to update-scenarios
  ...
  if scenario != 0 [
    ifelse year = item 0 scenario-res-levs and day = item 1 scenario-res-levs
    [
      let res-delta ( item 2 scenario-res-levs ) * magnitude * max-resources-base
  ...
  let days-rec [121 152 182 213 244 274]

  if member? day days-rec [
    let T-B precision ( mean [ ITV-BMR ] of turtles ) 3
    let T-R precision ( mean [ ITV-repro ] of turtles ) 3
    let T-G precision ( mean [ ITV-growth ] of turtles ) 3
    let sd-T-B precision ( standard-deviation [ ITV-BMR ] of turtles ) 3
    let sd-T-R precision ( standard-deviation [ ITV-repro ] of turtles ) 3
    let sd-T-G precision ( standard-deviation [ ITV-growth ] of turtles ) 3
    let BM precision ( mean [ mass ] of turtles * 1000 ) 3
   ; let IBI -9999
   ; if length IBI-list > 0 [ set IBI precision ( mean IBI-list ) 3 ]
   ; set IBI-list []
    let pop-abun count turtles + sum [n-off] of turtles

    let month-vals (list year day T-B T-R T-G sd-T-B sd-T-R sd-T-G BM pop-abun)
    ; print month-vals

    set scen-outs-list lput month-vals scen-outs-list
    ; print scen-outs-list
    ]
...
"food" 1.0 0 -7500403 true "" "if ticks > 1 [ plotxy ticks max-resources / max-resources-base ]"
...
"tot-res" 1.0 0 -14835848 true "" "if ticks > 1 [ if remainder ticks 48 = 0 [ plotxy ticks ( sum [resource-level] of patches / 105000 )]]"
...
@#$#@#$#@
<experiments>
  <experiment name="CalRuns" repetitions="200000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>growth-lm-prob-const</metric>
    <metric>growth-lm-prob-mid</metric>
    <metric>preg-prob-const</metric>
    <metric>preg-prob-mid</metric>
    <metric>lact-prob-const</metric>
    <metric>lact-prob-mid</metric>
    <metric>surv-prob-const</metric>
    <metric>surv-prob-mid</metric>
    <metric>surv-mod-embryo</metric>
    <metric>surv-mod-off</metric>
    <metric>p1-FMAB</metric>
    <metric>p2-BMxLS</metric>
    <metric>p34-BMLMxA</metric>
    <metric>p567-dsbxMMFILM</metric>
    <metric>p8910-LSxpFIEUMEO</metric>
    <metric>p11-PMxLS</metric>
    <metric>p12-LSb</metric>
    <metric>p13-LSw</metric>
    <metric>p14-PwxBM</metric>
    <metric>p15-BF</metric>
    <metric>p16-BF</metric>
    <metric>p17-FMRxBM</metric>
    <enumeratedValueSet variable="max-resources-base">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="resources-dynamic?">
      <value value="true"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="perc-resource-patches">
      <value value="0.75"/>
    </enumeratedValueSet>
  ...
  <experiment name="experiment" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>year = 25</exitCondition>
    <metric>pop-list</metric>
    <metric>off-list</metric>
    <enumeratedValueSet variable="max-resources-base">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="resources-dynamic?">
  ...
  <experiment name="Densities" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="77290"/>
    <metric>dens-list</metric>
    <steppedValueSet variable="max-resources-base" first="1" step="2" last="15"/>
    ...
    <enumeratedValueSet variable="n-food-c">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="print?">
      <value value="false"/>
    ...
    <steppedValueSet variable="r-growth-ts" first="0.01" step="0.005" last="0.05"/>
  ...
  <experiment name="DensityFinal" repetitions="250" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="77290"/>
    <metric>dens-list</metric>
  ...
  <experiment name="Evaluation" repetitions="150" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>p17-FMRxState              ; Pattern 17. State-dependent field metabolic rate</metric>
    <metric>p18-IRxState               ; Pattern 18. State-dependent food consumption</metric>
    <metric>p19-surv                   ; Pattern 19. Survival rates</metric>
    <metric>p20-BMadult                ; Pattern 20. Adult body mass</metric>
    <metric>p21-age1stbirth            ; Pattern 21. Age at first birth</metric>
    <metric>p22-BMRadult               ; Pattern 22. Mass-specific basal metabolic rate</metric>
    <metric>p23-LSb                    ; Pattern 23. Litter size</metric>
    <metric>p24-LpY                    ; Pattern 24. Litters per year</metric>
    <metric>p25-BMemb                  ; Pattern 25. Neonate body mass</metric>
    <metric>p26-BMwean                 ; Pattern 26. Weaning body mass</metric>
    <metric>p27-dens                   ; Pattern 27. Population density</metric>
    <enumeratedValueSet variable="max-resources-base">
      <value value="140"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pcolor-update?">
  ...
  <experiment name="CalCheckBMRlm" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>p1-FMAB</metric>
    <metric>p2-BMxLS</metric>
    ...
    <metric>p12-LSb</metric>
    <metric>p13-LSw</metric>
    <metric>p14-BF</metric>
    <metric>p15-BF</metric>
    <metric>p16-FMRxBM</metric>
    <metric>dens-mean</metric>
    <enumeratedValueSet variable="max-resources-base">
      <value value="140"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pcolor-update?">
  ...
  <experiment name="EvalCheckBMRlm" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>p17-FMRxState</metric>
    <metric>p18-IRxState</metric>
    <metric>p19-surv</metric>
    <metric>p20-BMadult</metric>
    <metric>p21-age1stbirth</metric>
    <metric>p22-BMRadult</metric>
    <metric>p23-LSb</metric>
    <metric>p24-LpY</metric>
    <metric>p25-BMemb</metric>
    <metric>p26-BMwean</metric>
    <metric>p27-dens</metric>
    <enumeratedValueSet variable="max-resources-base">
      <value value="140"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="pcolor-update?">
  ...
  <experiment name="EcoEvoInitScenarios" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>scen-outs-list</metric>
    <enumeratedValueSet variable="pcolor-update?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="print?">
    ...
    <enumeratedValueSet variable="magnitude">
      <value value="0.1"/>
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ITV-var-BMR">
