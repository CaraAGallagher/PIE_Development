...
extensions [ profiler csv matrix ]
...
globals [
  ...
  mean-res-year              ; List of yearly values for average resource levels
  res-yr                     ; Random year pulled after year 23
...
to setup-landscape
  ...
  ifelse debug = "Scen" [
    ...
    let yr-val -99
    set yr-val item year mean-res-year
    ;print yr-val

    let start-val -99
    set start-val matrix:get scenario-res-levs 0 0
    ;print start-val

   ; set max-resources max-resources-base * ( yr-val * 2 ) * (start-val)
    set max-resources max-resources-base * (( yr-val + start-val ) * 2)
    ;print max-resources
...
to setup-scenarios
  ...
  if debug = "Scen"
  [
    let file (word "NDVIInputScenariosYr_" site experiment)

    file-open (word path file)

    ; list of yearly average values (btw 0 - 2)
    ; columns are days (90-270)
    set mean-res-year csv:from-row file-read-line

    ; Array of relative start values (day 91) and daily changes (all other days) of resource levels (btw 0 - 2)
    ; rows are years (1-23 or 2000 - 2022) and columns are days (91-276)
    set scenario-res-levs csv:from-file (word path "NDVIInputScenariosIA_" site ".csv")
    set scenario-res-levs matrix:from-row-list scenario-res-levs
...
;  ]
  ...
  ; Scenario updates
  if debug = "Scen" [ update-scenarios-upd ]
...
to storage-dynamics
  ...
  [
    ...
    if debug = 10 or debug = "Scen" and (year > out-year - 5 and year <= out-year) or year = final-year [
...
;  ]
  ...
  ; For scenarios
  if debug = "Scen" and (year > out-year - 5 and year <= out-year) or year = final-year
  [
...
to update-scenarios-upd
; update environmentals

  if debug = "Scen" [
    if day = year-start and year >= 24 [ set res-yr 1 + random 23 ]

    let yr-val -99
    set yr-val item (year - 1) mean-res-year

    let yr year
    if year >= 24 [ set yr res-yr ]

    let day-val -99
    set day-val matrix:get scenario-res-levs (yr - 1) (day - 90)
    ;print day-val

    ifelse day = year-start [
      set max-resources max-resources-base * ( yr-val * 2 ) * day-val
      ; SHIFTED set max-resources max-resources-base * (( yr-val + day-val ) * 2)
      if max-resources < 0 [ set max-resources 0 ]
      ask resource-cells [ set resource-level  max-resources ]

    ]
    [
      let res-delta max-resources-base * ( yr-val * 2 ) * day-val
      ; SHIFTED let res-delta ( day-val * 2 ) * max-resources-base
  ...
  let days-rec-abun [121 152 182 213 244 272]
  ...
  if year = out-year or year = final-year [
    ...
    if member? day days-rec-abun [
      ...
      let BMs-mean precision (mean BMs-day) 3

      ; Body condition (percent fat mass) of adults
      let BCs-day [precision (storage-level) 3] of turtles with [age > t-mature]
      let BCs-mean precision (mean BCs-day) 3

      ; Total energy use of adults
      let FMRs-day [round (sum daily-m-tot-no-prod)] of turtles with [age > t-mature]
      let FMRs-mean precision (mean FMRs-day) 3

      ; Locomotive costs of adults
      let locos-day [round daily-m-move] of turtles with [age > t-mature]
      let locos-mean precision (mean locos-day) 3

      ; Total reproductive costs of adults
      let repros-day [round (daily-m-lact + daily-m-preg)] of turtles with [age > t-mature]
      let repros-mean precision (mean repros-day) 3

      ; Lean mass growth costs of juveniles
      let growths-day [round daily-m-lean-mass] of turtles with [age <= t-mature + 10]
      let growths-mean "NA"
      if length growths-day != 0 [ set growths-mean precision (mean growths-day) 3 ]

      let LSW-mean "NA"
      if length LSW-temp-list > 0 [ set LSW-mean precision (mean LSW-temp-list) 3 ]
      set LSW-temp-list []

      let neo-BM-mean "NA"
      if length neo-BM-temp-list > 0 [ set neo-BM-mean precision (mean neo-BM-temp-list) 3 ]
      set neo-BM-temp-list []

      let wean-BM-mean "NA"
      if length wean-BM-temp-list > 0 [ set wean-BM-mean precision (mean wean-BM-temp-list) 3 ]
      set wean-BM-temp-list []

      let day-means (list year day BMs-mean BCs-mean FMRs-mean locos-mean repros-mean growths-mean LSW-mean neo-BM-mean wean-BM-mean)
      set day-mean-list lput day-means day-mean-list
    ]

    if day = year-end - 1 [
      let LRSs [LRS] of turtles
      set LRS-temp-list sentence LRS-temp-list LRSs
      set LRS-temp-list remove 0 LRS-temp-list
      let LRS-mean precision (mean LRS-temp-list) 3
      set LRS-temp-list []

      let first-bs [age-1st-birth] of turtles
      set age-1st-birth-temp-list sentence age-1st-birth-temp-list first-bs
      set age-1st-birth-temp-list remove 0 age-1st-birth-temp-list
      let age-1st-birth-mean precision (mean age-1st-birth-temp-list) 3
      set age-1st-birth-temp-list []

      let LPYs [litters-per-year] of turtles
      set LPY-temp-list sentence LPY-temp-list LPYs
      set LPY-temp-list remove 0 LPY-temp-list
      let LPY-mean precision (mean LPY-temp-list) 3
      set LPY-temp-list []

      let age-at-death-mean precision (mean age-at-death-temp-list) 3
      set age-at-death-temp-list []

      let year-means (list year LRS-mean age-1st-birth-mean LPY-mean age-at-death-mean)
      set year-mean-list lput year-means year-mean-list
    ]
  ]
...
<experiments>
  ...
  <experiment name="NDVI_scenarios_IA" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>BM-list-base</metric>
    <metric>BC-list-base</metric>
    ...
    <metric>repro-list</metric>
    <metric>growth-list</metric>
    <enumeratedValueSet variable="surv-mod-embryo">
      <value value="0.66"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-resources-base">
      <value value="140"/>
    ...
    <enumeratedValueSet variable="lact-prob-mid">
      <value value="0.12"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="final-year">
      <value value="99"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="surv-mod-off">
      <value value="0.35"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="surv-prob-mid">
      <value value="0.01"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ITV-var-BMR">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="preg-prob-mid">
      <value value="0.02"/>
    </enumeratedValueSet>
    <steppedValueSet variable="sites" first="1" step="1" last="10"/>
    <enumeratedValueSet variable="r-growth-ts">
    ...
    <enumeratedValueSet variable="growth-lm-prob-const">
      <value value="47.23"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="overwinter-skip?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="preg-prob-const">
      <value value="13.97"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="debug">
      <value value="&quot;Scen&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="growth-lm-prob-mid">
      <value value="0.1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-animals">
      <value value="100"/>
    ...
    <enumeratedValueSet variable="lact-prob-const">
      <value value="19.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="home-ranging?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="surv-prob-const">
      <value value="59.45"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="fragmentation-level">
      <value value="0.9"/>
  ...
  <experiment name="NDVI_scenarios_IA" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>BM-list-base</metric>
    <metric>BC-list-base</metric>
  ...
  <experiment name="NDVI_scenarios_IA_10yrs" repetitions="60" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>day-mean-list</metric>
    <metric>year-mean-list</metric>
    <metric>abun-outs-list</metric>
    <enumeratedValueSet variable="surv-mod-embryo">
      <value value="0.66"/>
    </enumeratedValueSet>
